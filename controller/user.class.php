...
class User
{
    ...
    {
        $sql = "SELECT * FROM m_prefix WHERE 1=1 and prefix_status='1'";
        $response = db_query($sql);
        return $response;
    }
    ...
    public static function getUserAll($depId, $loginType)
    {
        if ($loginType == "0") {
         ...
         $sql = "select usr_fname,usr_lname,usr_id,usr_type,dep_name,pos_name,is_manager,prefix_name,m_user.dep_id,dep_name,usr_status FROM m_user
